...
class TestOptimizerE2E:
    ...
    def test_milp_solution_satisfies_ash_volatile_constraints(self):
        """MILP 解满足灰分/挥发分/硫分约束，粘结 status 为 null（参考值）"""
    ...
    def test_milp_solution_is_feasible_and_reasonable(self):
        """MILP 求解 8 煤种：手动配比达标 → MILP 亦可行 → 全部达标且成本合理"""
        results = run_js("""
            coals = getDefaultCoals();
        ...
        # 默认手动配比（实际分母=8）：volatile=31.4≥28、glue=78.5≥75 → 达标
        assert r["manualFeasible"] is True, "默认手动配比应达标（实际分母模型）"
        ...
        assert 300 <= r["optCost"] <= 1500, f"优化成本 {r['optCost']} 应在合理范围"
